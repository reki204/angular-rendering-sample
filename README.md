...
**Angular 19 のハイブリッドレンダリング**機能を活用して、**CSR（クライアントサイドレンダリング）**、**SSR（サーバーサイドレンダリング）**、**SSG（静的サイト生成）**、**ISR（静的サイト再生成）**、**PPR（部分プリレンダリング）** の各レンダリングパターンを実装したアプリケーションです。
...
- **対応レンダリングモード**
  ...
  - **PPR:** ビルド時にページを生成しつつ、動的コンテンツはクライアント側で取得します。
...
- **Partial Prerendering (PPR)** – 一部を静的に生成し、残りをクライアントで取得
...
- src/app/
  ...
  - 各レンダリングモード用コンポーネント:
    ...
    - PPR: 静的に生成したページに、クライアント側で取得する動的部分を組み合わせたコンポーネント
  - render-info.service.ts: 現在のレンダリングモード（CSR/SSR/SSG/ISR/PPR）を判定するためのサービス
