...
**Angular 20 のハイブリッドレンダリング**機能を活用して、**CSR（クライアントサイドレンダリング）**、**SSR（サーバーサイドレンダリング）**、**SSG（静的サイト生成）**、**ISR（静的サイト再生成）** の各レンダリングパターンを実装したアプリケーションです。

## 特徴

- **Angular 20 ハイブリッドレンダリング**
  Angular 20 の機能を用い、CSR、SSR、SSG、ISR の各パターンを実装。
...
- **Angular 20** – 最新のフレームワークをベースに、ハイブリッドレンダリング機能を活用
...
- [Angular CLI](https://angular.io/cli) v 20.0.0
